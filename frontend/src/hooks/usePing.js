    ...
    const doPing = () => {
        ...
        for (let i = 0; i < path.length - 1; i++) {
            setTimeout(() => {
                ...
                setActivePingNode(path[i]);
            }, i * 400 / animSpeed);
        }
        
        // Highlight destination when received
        setTimeout(() => {
            setActivePingNode(path[path.length - 1]);
            setTimeout(() => setActivePingNode(null), 400 / animSpeed);
        }, (path.length - 1) * 400 / animSpeed);
    };
