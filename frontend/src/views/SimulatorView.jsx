...
// ─────────────────────────────────────────────────────────────────────────────
// SVG Network Graph sub-component
// ─────────────────────────────────────────────────────────────────────────────
...
export default function SimulatorView() {
  ...
  useEffect(() => {
    if (isRunning && iteration > 0) {
      // Simple convergence check: compare tables from last two iterations
      // (real check lives in useRoutingTables / algorithms.js)
      const converged = iteration >= nodes.length + 1;   // placeholder
      if (converged) {
        stop();
    ...
    <div style={S.root}>
      ...
      <div style={S.body}>
        ...
        <div style={S.graphPanel}>
          ...
          {/* Floating Tools Card (add node / link) */}
          <div style={{
            position: "absolute",
